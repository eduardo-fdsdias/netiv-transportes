...
function App({ pathname = "/" }: { pathname?: string }) {
  ...
  const requestLink = () => waLink(whatsAppSourceMessage(attribution))
  ...
  return <>
    ...
    <main id="conteudo" tabIndex={-1}>
      ...
      <section id="servicos" className="section services-section"><div className="shell">
        ...
        <div className="services-grid">{services.map(({icon:Icon,title,text,cta}) => <article className="service-card" key={title}><span className="service-icon"><Icon size={23}/></span><h3>{title}</h3><p>{text}</p><a href={requestLink()} target="_blank" rel="noopener noreferrer">{cta} →</a></article>)}</div>
      </div></section>
      ...
      <section id="faq" className="section faq-section"><div className="shell"><div className="section-heading"><span className="section-kicker">Perguntas frequentes</span><h2>Dúvidas sobre guincho 24 horas</h2><p>Respostas objetivas para decidir e pedir atendimento com segurança.</p></div><div className="faq-visible">{pageFaqs.map(([question,answer])=><details key={question}><summary>{question}</summary><p>{answer}</p></details>)}</div><div className="center-cta"><a className="button button-dark" href={requestLink()} target="_blank" rel="noopener noreferrer">Tirar dúvida no WhatsApp</a></div></div></section>

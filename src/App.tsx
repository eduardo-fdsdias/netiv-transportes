...
const faqs = [
  ...
  ["Quais cidades vocês atendem?", "Atendemos um raio aproximado de 18 km a partir do CEP 09820-670, incluindo São Bernardo do Campo e cidades do ABC. Serra de Santos, Praia Grande, Cubatão, Guarujá e outras regiões são atendidas conforme disponibilidade de rota."],
...
function App({ pathname = "/" }: { pathname?: string }) {
  ...
  return <>
    ...
    <main id="conteudo" tabIndex={-1}>
      ...
      <section id="areas-atendidas" className="section areas-section"><div className="shell"><div className="section-heading"><span className="section-kicker">Atendimento regional 24 horas</span><h2>Regiões atendidas</h2><p>Atendimento local em um raio aproximado de 18 km a partir do CEP 09820-670, além de rotas para a Baixada Santista conforme disponibilidade.</p></div><nav className="region-links" aria-label="Páginas de regiões e rotas">{regions.map(item => <a key={item.slug} href={`${BASE_PATH}${item.slug}/`}>{item.title} →</a>)}</nav><div className="coverage-label"><span>Base local</span><strong>{ADDRESS_DISPLAY}</strong><small>Raio aproximado de 18 km</small></div><div className="local-grid">{cities.map(city=><article key={city}><MapPin size={22}/><h3>Guincho em {city}</h3><p>Atendimento 24h para carros, motos e veículos leves.</p><a href={waLink(`Olá! Preciso de guincho em ${city}. Minha localização é: `)} target="_blank" rel="noopener noreferrer">Consultar disponibilidade →</a></article>)}</div><div className="coast-heading"><span>Rotas para o litoral</span><h2>Atendimento na Serra e Baixada Santista</h2><p>Consulte disponibilidade para remoções, transportes e deslocamentos entre o ABC e o litoral.</p></div><div className="coast-grid">{coastAreas.map(area=><article key={area}><Truck size={21}/><div><h3>Guincho para {area}</h3><p>Atendimento sob consulta de rota e disponibilidade.</p><small>Consulte o tempo estimado de deslocamento ao solicitar.</small></div><a href={waLink(`Olá! Preciso consultar um guincho para ${area}. Origem e destino: `)} target="_blank" rel="noopener noreferrer">Consultar rota →</a></article>)}</div><div className="extra-areas"><strong>Outras regiões sob consulta:</strong><div className="extra-area-links">{extraCities.map(city=><a key={city} href={waLink(`Olá! Preciso consultar um guincho para ${city}. Minha localização é: `)} target="_blank" rel="noopener noreferrer">{city} <span>→</span></a>)}<a href={waLink("Olá! Preciso consultar um guincho para outra região. Minha localização é: ")} target="_blank" rel="noopener noreferrer">Outra região <span>→</span></a></div></div></div></section>
